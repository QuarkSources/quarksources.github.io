    ...
    def update(self):
        ...
        for data in self.src_data:
            ...
            if isinstance(parser, AltSourceParser):
                ...
                if version.parse(app["version"]) < version.parse(parser.version):
...
class GithubParser:
    def __init__(self, url: str = None, repo_author: str = None, repo_name: str = None, ver_parse = lambda x: x.lstrip("v"), include_pre: bool = False):
        """
        Supply either the api url, or the repo_author and repo_name.
        Include a lambda function for ver_parse if needed.
        """
        if url is not None:
